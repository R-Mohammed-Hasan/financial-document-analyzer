...
class AnalysisHistoryResponse(BaseModel):
    ...
    analyses: List[AnalysisHistoryRecord] = Field(
...
class AnalysisStatusResponse(BaseModel):
    ...
    estimated_time_remaining: int = Field(
...
class FileValidationResponse(BaseModel):
    ...
    file_info: Optional[Dict[str, Any]] = Field(
        None, description="File information if valid"
    )
...
class AnalysisMetricsResponse(BaseModel):
    ...
    most_analyzed_file_type: str = Field(
        ..., description="Most common file type analyzed"
    )
    analysis_trends: Dict[str, int] = Field(
        ..., description="Analysis trends by time period"
    )
...
class BatchAnalysisRequest(BaseModel):
    ...
    options: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Additional batch options"
    )
...
class BatchAnalysisResponse(BaseModel):
    ...
    success: bool = Field(
        ..., description="Whether batch analysis was initiated successfully"
    )
...
class FinancialMetricData(BaseModel):
    ...
    period: str = Field(
        ..., description="Time period identifier, e.g., 'Q1 2023' or '2023'"
    )
    ...
    qoq_change: Optional[float] = Field(
        None, description="Quarter-over-quarter change percentage"
    )
...
class FinancialAnalysisCreate(BaseModel):
    ...
    analysis_type: str = Field(
        ..., description="Type of analysis (e.g., 'revenue', 'eps', 'comparative')"
    )
    ...
    data: Dict[str, Any] = Field(
        ..., description="Analysis data in a structured format"
    )
...
class RevenueTrendsRequest(BaseModel):
    ...
    time_period: TimePeriod = Field(
        TimePeriod.QUARTERLY, description="Time period for the analysis"
    )
    segments: Optional[List[str]] = Field(
        None, description="Segments to include (e.g., product lines, geographies)"
    )
...
class RevenueTrendsResponse(BaseModel):
    ...
    time_period: TimePeriod = Field(
        ..., description="Time period used for the analysis"
    )
    metrics: List[FinancialMetricData] = Field(
        ..., description="Revenue metrics over time"
    )
    segments: Optional[Dict[str, List[FinancialMetricData]]] = Field(
        None, description="Revenue by segment if segments were specified"
    )
    total_growth: Optional[float] = Field(
        None, description="Total growth percentage over the period"
    )
...
class EPSAnalysisRequest(BaseModel):
    ...
    include_guidance: bool = Field(
        True, description="Whether to include guidance data if available"
    )
    compare_to_analyst_expectations: bool = Field(
        False, description="Whether to compare with analyst expectations if available"
    )
...
class EPSAnalysisResponse(BaseModel):
    ...
    eps_metrics: List[FinancialMetricData] = Field(
        ..., description="EPS metrics over time"
    )
    guidance: Optional[List[Dict[str, Any]]] = Field(
        None, description="Guidance data if available and requested"
    )
    analyst_expectations: Optional[List[Dict[str, Any]]] = Field(
        None, description="Analyst expectations if available and requested"
    )
...
class ComparativeAnalysisRequest(BaseModel):
    ...
    time_period: TimePeriod = Field(
        TimePeriod.YEARLY, description="Time period for the analysis"
    )
    include_percent_change: bool = Field(
        True, description="Whether to include percentage changes"
    )
...
class ComparativeAnalysisResponse(BaseModel):
    ...
    heatmap_data: Optional[Dict[str, Any]] = Field(
        None, description="Formatted data for heatmap visualization"
    )
