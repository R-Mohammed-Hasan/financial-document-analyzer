"""
Centralized logging configuration for core-backend.
...
def setup_logging(
    ...
    Args:
        ...
        log_dir: Directory for log files (relative to core-backend root by default)
    """
    ...
    # Resolve logs directory relative to this core-backend package by default
    # core-backend/app/core/logging_config.py -> core-backend/
    base_dir = Path(__file__).resolve().parents[2]
    log_path = (base_dir / log_dir) if not Path(log_dir).is_absolute() else Path(log_dir)
    ...
    logger.info(
        "Logging configured (core-backend)",
        extra={
            "extra_fields": {
